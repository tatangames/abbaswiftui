...
class InsigniasPorGanarViewModel: ObservableObject {
    @Published var jsonResponse: JSON?
    @Published var loadingSpinner: Bool = false
    @Published var isRequestInProgress: Bool = false
    @Published var error: Error?
    @Published var insignias: [ModeloInsigniasListado] = []
    
    private let disposeBag = DisposeBag()
     
    func insigniasPorGanarRX(idToken: String, idCliente: String, idiomaApp: Int, completion: @escaping (Result<JSON, Error>) -> Void) {
        ...
        let encodeURL = apiListaInsigniasPorGanar
